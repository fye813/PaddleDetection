...
class Pipeline(object):
    ...
    def __init__(self, args, cfg):
        ...
        self.area_threads = args.area_threads  # 新しい引数を設定
...
class PipePredictor(object):
    ...
    def __init__(self, args, cfg, is_video=True, multi_camera=False):
        ...
        self.area_threads = args.area_threads
    ...
    def predict_video(self, video_file, thread_idx=0):
        ...
        with open(csv_output_path, mode='w', newline='') as csv_file:
            fieldnames = [
                ...
                'Last Frame Center X',
                'Last Frame Center Y',
                'First Frame Bottom X',
                'First Frame Bottom Y',
                'Last Frame Bottom X',
                'Last Frame Bottom Y'
                ]
            ...
            for track_id, frames in self.detection_tracking_info.items():
                ...
                # 最初のフレームでの中心座標を取得
                first_bottom = frames[0]["bottom"]
                first_bottom_x, first_bottom_y = first_bottom
                first_bottom_x = round(first_bottom_x, 1)
                first_bottom_y = round(first_bottom_y, 1)

                # 最後のフレームでの中心座標を取得
                last_bottom = frames[-1]["bottom"]
                last_bottom_x, last_bottom_y = last_bottom
                last_bottom_x = round(last_bottom_x, 1)
                last_bottom_y = round(last_bottom_y, 1)
                ...
                writer.writerow({
                    ...
                    'First Frame Bottom X': first_bottom_x,
                    'First Frame Bottom Y': first_bottom_y,
                    'Last Frame Bottom X': last_bottom_x,
                    'Last Frame Bottom Y': last_bottom_y
                })
        ...
        with open(csv_output_path, mode='w', newline='') as csv_file:
            ...
            for frame_id, tracks in self.frame_info.items():
                ...
                # 設定値
                area_threads = self.area_threads

                # 条件を満たす数値をカウント
                area_count = 0

                for track in tracks:
                    if track['bottom'][0]>=area_threads[0] and track['bottom'][1]>=area_threads[1]:
                        area_count += 1
                ...
                writer.writerow({
                    ...
                    'in count': in_count,
                    'out count': out_count,
                    'Detection ID Count Bottom': area_count
                })
            ...
            for frame_id, tracks in self.frame_info.items():
                ...
                for track in tracks:
                    writer.writerow({
                        ...
                        'Bottom X': round(track["bottom"][0], 2),
                        'Bottom Y': round(track["bottom"][1], 2)
                    })
    ...
    def visualize_video(self,
        ...
        if mot_res is not None:
            ...
            for track_id, box, score in zip(ids, boxes, scores):
                ...
                # 下辺の中心座標の計算
                bottom_x = xmin + width / 2
                bottom_y = ymin + height

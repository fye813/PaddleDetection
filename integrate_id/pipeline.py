...
def merge_similar_detections(df, max_frame_diff,threshold):
    ...
    for detection_id in unique_ids:
        ...
        if current_id_df.empty: continue
        ...
        for target_id in potential_ids_df['Detection ID'].unique():
            ...
            # print(f"{detection_id} ← {target_id} (distance: {distance}, allowed: {allowed_distance})")
            df.loc[df['Detection ID'] == target_id, 'Detection ID'] = detection_id

            integrated_ids.append(detection_id)
            integrate_cnt += 1

            # 統合された後、last_time と last_position を更新
            last_frame = target_id_df['Elapsed Seconds'].max()
            last_position = target_id_df[['Center X', 'Center Y']].iloc[-1].values
